...
1.2 A reference to another definition is `id@version`. The version MAY be a caret range (`^1.2`); it is resolved **once, when a run starts**, and the run records the exact versions it pinned. A run MUST NOT change definition versions while in flight. *(UC-VERSION)*
...
- If `deciders` is absent, the gate is decided by a person holding the gate's permission.
- A decider that cannot run, or gives a verdict outside its `may` list, counts as `INDETERMINATE`.
