...
| V9 gates | a `PERMIT` or `DENY` with no route; an `INDETERMINATE` with a route; an unknown `kind`; an agent decider not returning `decision@1`; an agent decider that could review its own output; `person_required_when` with no `person` decider; a gate with no `asks`; `note_into` targeting a non-`REPLACE` channel (D27) |
