...
| V9 gates | a `PERMIT` or `DENY` with no route; an `INDETERMINATE` with a route; an unknown `kind`; `kind: INPUT` (spec-legal, not yet engine-executed, D24); an agent decider not returning `decision@1`; an agent decider that could review its own output; `person_required_when` with no `person` decider; a gate with no `asks` |
