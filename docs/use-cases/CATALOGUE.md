...
| UC-TOOL-CONTROLS | Mandatory, enforced controls | Every Tool declares at least one control (a profile, conventions, fitness bar or policy) that resolves, and each control has a registered deterministic checker Tool; after a step the output is checked and the typed result is routed on. A control that does not resolve refuses to run; a non-deterministic Tool with no controls is refused as unevaluable. | FD ADR-0074 step contract (IDEF0 ICOM), `tools/*.tool.yaml` `controls`, `check-instance` as the `profile` checker; ME `context.{input,processing,output}` standards; BR step `controls` (IDEF0) | partial (FD: checked to exist, graded only in evals) |
| UC-TOOL-TYPED-IO | Typed Tool inputs and outputs | A Tool declares typed inputs and outputs (profiles); a step maps run state onto them, and a mapping that does not type-check is refused before the run. | FD `tool.schema.json` inputs/output + `tool_step.py:43-134`; BR IDEF0 `input_artifacts`/`output_artifacts` (untyped names) | partial (FD) |
